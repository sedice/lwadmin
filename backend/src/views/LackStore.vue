...
export default {
  ...
  methods: {
    ...
    getTitle (item) {
      ...
      var day = time.getDate();
      if (day < 10) day = '0' + day;
      return shop + "_" + creator + "_缺货统计单_" + year + month + day + "_" + createindex;
    },
    getData() {
      // 获取表格数据
      ...
      var url = `/api/lackstore?page=${page}&pagesize=${pagesize}`;
      // 按创建者删选
    ...
    onDeleteData (row) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          ...
          this.$axios.delete(`/api/lackstore/${row._id}`).then(res => {
            this.$message({
              type: 'success',
              message: '删除成功'
            });
