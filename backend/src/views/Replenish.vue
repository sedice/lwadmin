...
export default {
  ...
  methods: {
    ...
    clickSerch () {
      ...
      console.log(this.serchRule)
      this.getData();
    },
    getDetailData (row) {
      var url = `/api/replenish/${row._id}`;
      this.$axios.get(url).then(res => {
        this.dialogDetailConf.datagroup = res.data.datagroup.map((name) =>{return {name}});
    ...
    getTitle (item) {
      ...
      var day = time.getDate();
      if (day < 10) day = '0' + day;
      return shop + "_" + creator + "_补货单_" + year + month + day + "_" + createindex;
    },
    getData() {
      // 获取表格数据
      ...
      var url = `/api/replenish?page=${page}&pagesize=${pagesize}`;
      // 按创建者删选
    ...
    onDeleteData (row) {
      this.$confirm('此操作将永久删除该记录, 是否继续?', '提示', {
          ...
          this.$axios.delete(`/api/replenish/${row._id}`).then(res => {
            this.$message({
              type: 'success',
              message: '删除成功'
            });
