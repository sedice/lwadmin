...
router.get("/batch_record", async function (ctx, next) {
  var { time_from, time_to, type = "out" } = ctx.query;
  const isOut = type === "out" ? true : false;
  time_from = parseInt(time_from);
  time_to = parseInt(time_to);
  var data = await getBatchRecordData(
    isOut ? batchRecordModelOut : batchRecordModelIn,
    time_from,
    time_to
  );
...
async function getBatchRecordData(model, time_from, time_to) {
  const dataGroup = await model
    .find()
    .where("createtime")
    .gte(time_from)
    .lte(time_to)
  ...
  const batchNum = 10;
  const retArray = [];
  retArray.push(getBatchRecordTitle(batchNum));
  dataGroup.forEach((data, index) => {
    getBatchRecordRowData(data, retArray);
  });
  return retArray;
}

function getBatchRecordRowData(data, array) {
  data.datagroup.forEach((item) => {
    ...
    group.push(
      `${data.shop}_${data.creator}_${createtime}_${data.createindex}`
    ); // 任务单号
...
// store表的标题
function getBatchRecordTitle(batchNum) {
  var array = [];
  array.push("序号");
  array.push("日期");
  array.push("门店");
