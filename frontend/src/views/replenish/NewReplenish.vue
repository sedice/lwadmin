<template>
  <div>
    ...
    <div class = "container" style = 'margin-top:50px;'>
      <p class = "promptTitle">请勾选出补货商品</p>
      <checklist  
        label-position = "left" 
        required 
        :options = "goods_group" 
        v-model = "replenish_group" 
      ></checklist>
    </div>
...
export default {
  ...
  methods :{
    ...
    clickCommit () {
      if (this.replenish_group.length == 0) {
      ...
      this.$http.post('/api/replenish',{
        datagroup:this.replenish_group
      })
      .then((res)=>{
        this.$vux.toast.show({
          text:"提交成功"
    ...
    getGoodsFromServer () {
      this.$http.get('/api/replenish/template')
        .then(({data})=>{
